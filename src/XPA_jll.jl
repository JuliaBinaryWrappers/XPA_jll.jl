...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import XPA_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function dev_jll()
        ...
        override_dir = joinpath(Pkg.devdir(), "XPA_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"XPA", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "XPA_jll", "src", "XPA_jll.jl"))
        @info("XPA_ll dev'ed out to /depot/dev/XPA_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("9dbca590-e19a-5566-89a8-3997bfd21c58"))
platforms = [Pkg.Artifacts.unpack_platform(e, "XPA", artifacts_toml) for e in artifacts["XPA"]]
...
if best_platform === nothing
    @debug("Unable to load XPA; unsupported platform $(triplet(platform_key_abi()))")
else
